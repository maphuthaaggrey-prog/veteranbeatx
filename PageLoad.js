window.addEventListener('DOMContentLoaded', function () {
    const AlbumImageContainer = document.getElementById('highlight-album-image');
...
window.addEventListener('DOMContentLoaded', function () {
    ...
    mixtapes.forEach((mixtape, index) => {
        if (index < 3) {
        const anchor = document.createElement('a');
        ...
        const singleElement = document.createElement('p');
        singleElement.id = 'mixtape-name';
        singleElement.innerText = `${mixtape.name}`

        const singleName = document.createElement('p');
        singleName.id = 'mixtape-name';
